...
function MouseControl () {
  ...
  this.up = function (e) {
    ...
    if (e.which === 1) this.release()
    else if (e.which === 3) this.right()
  }
  this.out = function () {
    if (console.isTransit) return
    ...
    if (zoom > 0) {
      if (console.isFull) {
        ...
        animation.event.push(
          new AnimateGoto([0, 0], 0.28 * console.scale, false)
        )
      }
    } else {
...
function AnimateGoto (gotAltz, zoomSpeed, forceZoom) {
  ...
  this.animate = function () {
    var speedFunction =
      Math.pow(Math.sin((Math.PI * this.turnLeft) / this.turnAll), 2) *
      this.deriviate

    if (this.turnLeft > 0) {
      if (this.forceZoom) console.forceAddScale(speedFunction * this.zoomSpeed)
      else console.addScale(speedFunction * this.zoomSpeed)
      console.addAzimuth(speedFunction * this.gotAltz[0])
      console.addAltitude(speedFunction * this.gotAltz[1])
      this.turnLeft--
    } else {
      if (
        Math.floor(console.scale) <= windowSize.getFullBall() &&
        console.isTransit
      ) { console.changeFullMap() }
      if (console.isTransit) console.isTransit = false
      this.turnLeft--
    }
  }
  this.slow = function (slowFactor) {
...
function AnimateRelease (obsAltz, releaseSpeed) {
  ...
  this.animate = function () {
    var speedFunction =
      Math.pow(Math.E, -(this.turnAll - this.turnLeft) / 2) * this.deriviate
