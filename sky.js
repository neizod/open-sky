...
function Background () {
  ...
  this.plotGround = function () {
    ...
    if (skyConsole.altitude !== -90 && skyConsole.scale < windowSize.getRadius()) { this.plotOverGround() }
  }
...
function Star (name, uname, ra, dec, mag, colour) {
  ...
  this.checkOnScreen = function (xy) {
    ...
    if (xy[0] > overSize * windowSize.halfWidth || xy[0] < -overSize * windowSize.halfWidth ||
            xy[1] > overSize * windowSize.halfHeight || xy[1] < -overSize * windowSize.halfHeight) { return false }
    return true
  }
