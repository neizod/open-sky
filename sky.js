...
function Console () {
  ...
  this.restore = function () {
    this.scale =
      windowSize.getRadius() > this.sw_scale
        ? windowSize.getRadius()
        : this.sw_scale
  ...
  this.addScale = function (zoom) {
    ...
    this.scale += zoom
    if (this.scale + zoom < windowSize.getRadius()) { this.scale = windowSize.getRadius() } else if (this.scale + zoom > this.mx_scale) this.scale = this.mx_scale
...
function Sky (rotation) {
  ...
  this.position = function (xy) {
    ...
    xyz[0] =
      ezGL.sgn(xy[0]) * Math.sqrt(1 - Math.pow(xyz[1], 2)) * Math.cos(posAng)
    xyz[2] =
      ezGL.sgn(xy[0]) * Math.sqrt(1 - Math.pow(xyz[1], 2)) * Math.sin(posAng)
    ...
    if (xyz[0] / Math.cos(rDec) <= 1) { var rRA = Math.asin(xyz[0] / Math.cos(rDec)) } else var rRA = Math.asin(1)

    var refDec = Math.asin(ref[2])
    if (ref[0] / Math.cos(refDec) <= 1) { var refRA = Math.asin(ref[0] / Math.cos(refDec)) } else var refRA = Math.asin(1)
...
function Background () {
  ...
  this.plotBackground = function () {
    ...
    ctx.fillRect(
      -windowSize.halfWidth,
      -windowSize.halfHeight,
      windowSize.width,
      windowSize.height
    )
  }
  this.plotGround = function () {
    ...
    if (console.altitude !== -90 && console.scale < windowSize.getRadius()) { this.plotOverGround() }
  }
  this.plotUnderGround = function () {
    ...
    plotGroundSet[++polygonNum] = [
      windowSize.halfWidth,
      windowSize.halfHeight + 1
    ]
    plotGroundSet[++polygonNum] = [
      -windowSize.halfWidth,
      windowSize.halfHeight + 1
...
function Star (name, uname, ra, dec, mag, colour) {
  ...
  this.cartesian = [
    Math.sin(this.rRA) * Math.cos(this.rDec),
  ...
  this.getRadius = function () {
    ...
    radius *= Math.pow(
      1.03,
      (console.scale - windowSize.getRadius()) / windowSize.getRadius()
    )
    return radius
  }
  ...
  this.checkOnScreen = function (xy) {
    ...
    if (
      xy[0] > overSize * windowSize.halfWidth ||
      xy[0] < -overSize * windowSize.halfWidth ||
      xy[1] > overSize * windowSize.halfHeight ||
      xy[1] < -overSize * windowSize.halfHeight
    ) { return false }
    return true
  }
  ...
  this.plotLine = function () {
    ...
    for (var c = 0; c < this.lineSet.length; c++) {
      ...
      for (var i = 0; i < this.lineSet[c].length; i++) {
        ...
        var myxy = this.plotSet[this.lineSet[c][i].id].plotPosition(
          this.rotation
        )
        ...
        for (var j = 0; j < this.lineSet[c][i].nbh.length; j++) {
          ...
          var desxy = this.plotSet[this.lineSet[c][ij].id].plotPosition(
            this.rotation
          )
          ctx.beginPath()
...
function drawSky () {
  ...
  if (true) {
    ...
    if (animation.event.length) { ctx.fillText(animation.event[0].obsAltz, -400, -35) }
